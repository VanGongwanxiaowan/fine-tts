...
我们使用👩‍✈️[Coqpit]进行配置管理。它在原生Python dataclasses的基础上提供基本的静态类型检查和序列化功能。以下是使用Coqpit的简单配置示例。
...
在文本转语音（TTS）中，每个模型都必须有一个配置类，该类会公开其生命周期所需的所有值。
...
它定义了模型架构、超参数、训练和推理设置。对于我们的模型，为了方便起见，我们将所有字段合并到一个配置类中。这可能看起来不是一个明智的做法，但却有助于更轻松地进行记录和可重复的实验。
...
在上面的示例中，ModelConfig() 是模型接收的最终配置，它包含模型所需的所有字段。
...
我们在TTS/<model_class>/configs/下托管了预定义的模型配置。尽管我们推荐使用统一的配置类，但对于自定义模型，只要提供了训练器、模型和推理API所需的所有字段，您可以按照自己的喜好对其进行分解。
